...
Raw VM artifacts default to a local temp directory and are summarized back into the issue. The summary must include exact paths, commands, exit codes, and residual risks. Only reviewed summaries should be committed to the repo; screenshots, raw responses, browser traces, action transcripts, and container ids must stay untracked unless a separate human-approved issue explicitly asks for durable evidence.

Use `CAPY_VM_ARTIFACT_DIR=<path>` only when a Supervisor needs a durable local artifact path. If that path is inside this repo, scrub it first and commit only summaries such as `RUN_DIGEST.md`.
