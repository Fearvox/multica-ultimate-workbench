...
: "${CLAUDE_RUNTIME_ID:?Set CLAUDE_RUNTIME_ID before creating agents}"
: "${CODEX_RUNTIME_ID:?Set CODEX_RUNTIME_ID before creating agents}"
: "${HERMES_RUNTIME_ID:?Set HERMES_RUNTIME_ID before creating agents}"
...
if [[ -n "${EXPECTED_MULTICA_PROFILE:-}" ]]; then
  check_target "MULTICA_PROFILE" "$MULTICA_PROFILE" "$EXPECTED_MULTICA_PROFILE"
fi
if [[ -n "${EXPECTED_MULTICA_WORKSPACE_ID:-}" ]]; then
  check_target "MULTICA_WORKSPACE_ID" "$MULTICA_WORKSPACE_ID" "$EXPECTED_MULTICA_WORKSPACE_ID"
fi
if [[ -n "${EXPECTED_CLAUDE_RUNTIME_ID:-}" ]]; then
  check_target "CLAUDE_RUNTIME_ID" "$CLAUDE_RUNTIME_ID" "$EXPECTED_CLAUDE_RUNTIME_ID"
fi
if [[ -n "${EXPECTED_CODEX_RUNTIME_ID:-}" ]]; then
  check_target "CODEX_RUNTIME_ID" "$CODEX_RUNTIME_ID" "$EXPECTED_CODEX_RUNTIME_ID"
fi
if [[ -n "${EXPECTED_HERMES_RUNTIME_ID:-}" ]]; then
  check_target "HERMES_RUNTIME_ID" "$HERMES_RUNTIME_ID" "$EXPECTED_HERMES_RUNTIME_ID"
fi
