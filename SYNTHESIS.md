...
| Workbench Max role ambiguity | Routing and burn-in scoreboards can overcount or misclassify Max as Inner/Outer Ring. | Treat Max as a preserved Special bench with human-explicit assignment only. |
| Autopilot-created issues lack a canonical workbench repo anchor | Agents cannot verify this repository's docs/specs from their assigned workdir. | Mitigated by the `Ultimate Workbench` project-bound GitHub repo resource; treat the old `file:///Users/0xvox/multica-ultimate-workbench` link as local-only fallback evidence, not the primary anchor. |
